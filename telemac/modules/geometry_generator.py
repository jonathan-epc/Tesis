...
class GeometryGenerator:
    ...
    def generate_geometry(
        ...
        **kwargs
        ...
        """
        np.random.seed(kwargs.get('random_seed'))
        ...
        if variation_function:
            variation = variation_function(xv, yv, channel_length, channel_width, **kwargs)
    ...
    def _normalize_variation(variation, min_var, max_var):
        ...
        return min_var + (variation - variation.min()) / (variation.ptp()) * (max_var - min_var)

    @staticmethod
    def _noise_variation(xv, yv, channel_length, channel_width, **kwargs):
        return gaussian_filter(np.random.rand(*xv.shape) * kwargs.get('noise_amplitude', 0.15), sigma=kwargs.get('noise_smoothness', 0.95))

    @staticmethod
    def _hybrid_variation(xv, yv, channel_length, channel_width, **kwargs):
        noise = GeometryGenerator._noise_variation(xv, yv, channel_length, channel_width, **kwargs)
        transition_point = int(xv.shape[0] // 2 + np.random.uniform(-0.4, 0.4) * channel_length)
    ...
    def _bump2d_variation(xv, yv, channel_length, channel_width, **kwargs):
        ...
        bump_center_x = channel_length / 2 + (np.random.uniform(-0.3, 0.3) * channel_length if randomize else 0)
        bump_center_y = channel_width / 2 + (np.random.uniform(-0.05, 0.05) * channel_width if randomize else 0)
        bump_width = kwargs.get('bump_width_x', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)
        bump_amplitude = kwargs.get('bump_amplitude', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)

        return bump_amplitude * np.exp(-((xv - bump_center_x) ** 2 + (yv - bump_center_y) ** 2) / (2 * bump_width ** 2))

    @staticmethod
    def _bump_variation(xv, yv, channel_length, channel_width, **kwargs):
        randomize = kwargs.get('randomize', True)
        bump_center = channel_length / 2 + (np.random.uniform(-0.3, 0.3) * channel_length if randomize else 0)
        bump_width = kwargs.get('bump_width_x', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)
        bump_amplitude = kwargs.get('bump_amplitude', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)
    ...
    def _step_variation(xv, yv, channel_length, channel_width, **kwargs):
        randomize = kwargs.get('randomize', True)
        step_position = kwargs.get('step_position', 0.5) + (np.random.uniform(-0.05, 0.05) if randomize else 0)
        step_height = kwargs.get('step_height', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)
    ...
    def _smooth_step_variation(xv, yv, channel_length, channel_width, **kwargs):
        randomize = kwargs.get('randomize', True)
        step_center = kwargs.get('step_position', 0.5) * channel_length + (np.random.uniform(-0.05, 0.05) * channel_length if randomize else 0)
        step_width = 0.01 * channel_length * (np.random.uniform(0.9, 1.1) if randomize else 1)
        step_height = kwargs.get('step_height', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)

        return step_height / (1 + np.exp(-(xv - step_center) / step_width))

    @staticmethod
    def _sinusoidal_variation(xv, yv, channel_length, channel_width, dimension, **kwargs):
        randomize = kwargs.get('randomize', True)
        amplitude = kwargs.get('sinusoidal_amplitude', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)
        frequency = kwargs.get('sinusoidal_frequency', 3) * (np.random.uniform(0.9, 1.1) if randomize else 1)

        return amplitude * np.sin(2 * np.pi * frequency * (xv if dimension == 'x' else yv) / (channel_length if dimension == 'x' else channel_width))

    @staticmethod
    def _sinusoidal_x_variation(xv, yv, channel_length, channel_width, **kwargs):
        return GeometryGenerator._sinusoidal_variation(xv, yv, channel_length, channel_width, 'x', **kwargs)

    @staticmethod
    def _sinusoidal_y_variation(xv, yv, channel_length, channel_width, **kwargs):
        return GeometryGenerator._sinusoidal_variation(xv, yv, channel_length, channel_width, 'y', **kwargs)

    @staticmethod
    def _bars_variation(xv, yv, channel_length, channel_width, **kwargs):
        randomize = kwargs.get('randomize', True)
        bars_frequency = kwargs.get('bars_frequency', 6 * 2 * np.pi / channel_length) * (np.random.uniform(0.9, 1.1) if randomize else 1)
        bars_amplitude = kwargs.get('bars_amplitude', 0.1) * (np.random.uniform(0.9, 1.1) if randomize else 1)
        phase_shift = kwargs.get('phase_shift', 1) * (np.random.uniform(0, 2 * np.pi) if randomize else 1)
        ...
        combined_sin = x_sin * (1 - transition) + np.sin(bars_frequency * xv + phase_shift + np.pi) * transition
    ...
    def _real_bars_variation(xv, yv, channel_length, channel_width, **kwargs):
        ...
        longitudinal_factor = 1 + longitudinal_variation * np.sin(2 * np.pi * xv / channel_length)
        bars *= longitudinal_factor

        irregularities = np.random.random(xv.shape) < irregularity_chance
        bars[irregularities] += np.random.uniform(-irregularity_magnitude, irregularity_magnitude, size=np.sum(irregularities))
