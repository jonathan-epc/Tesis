...
def main():
    ...
    try:
        ...
        if args.dry_run:
            logger.info(f"Dry run: would process files from {start_index} to {end_index}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--start', type=int, default=0, help='Start index for simulation files')
    parser.add_argument('--end', type=int, default=0, help='End index for simulation files')
    parser.add_argument('--bottom', type=str, help='Select a specific bottom value for simulation')
    parser.add_argument('--output-dir', type=str, default=OUTPUT_FOLDER, help='Output directory for simulation results')
    parser.add_argument('--log-level', type=str, default='INFO', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'], help='Set the logging level')
    parser.add_argument('--dry-run', action='store_true', help='Perform a dry run without actually running simulations')
...
def load_and_validate_parameters(file_path: str, logger) -> pd.DataFrame:
    try:
        ...
        missing_columns = [col for col in required_columns if col not in parameters.columns]
        if missing_columns:
            raise ValueError(f"Missing required columns in parameters file: {', '.join(missing_columns)}")
...
def determine_file_range(args: argparse.Namespace, steering_folder: str) -> Tuple[int, int]:
...
def filter_parameters_by_bottom(parameters: pd.DataFrame, selected_bottom: float, logger) -> pd.DataFrame:
    if 'BOTTOM' not in parameters.columns:
        logger.warning("'BOTTOM' column not found in parameters. Running all simulations.")
        return parameters

    filtered_params = parameters[parameters['BOTTOM'] == selected_bottom]
    
    if filtered_params.empty:
        logger.warning(f"No parameters found for bottom value {selected_bottom}. Running all simulations.")
        return parameters
    
    logger.info(f"Filtered parameters to include only bottom value {selected_bottom}. "
