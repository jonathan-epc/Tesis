...
class Config:
    ...
    extensions_to_include: set[str] = field(
    ...
    redaction_patterns: list[tuple[str, str]] = field(
        default_factory=lambda: [
            ...
            (
                r"\b(?:YARKO\s+NIÑO|LUIS\s+ZAMORANO|PAULA\s+AGUIRRE|VALENTIN\s+BARRIERE)[^\s]*",
                r"[NAME]",
            ),
            (r"Advisors:[^\n]*", r"Advisors: [NAMES]"),
    ...
    _compiled_patterns: list[tuple[Pattern, str]] = field(
        default_factory=list, init=False
    )
    _comment_pattern: Pattern = field(default=None, init=False)
    _latex_cleanup_patterns: list[tuple[Pattern, str]] = field(
        default_factory=list, init=False
    )
...
def _is_excluded(path: Path, root: Path, config: Config) -> bool:
    ...
    try:
        ...
        if any(
            rel_path.startswith(prefix) for prefix in config.path_prefixes_to_exclude
        ):
            return True
    except ValueError:
        pass
...
def generate_project_summary(config: Config):
    ...
    for root, dirs, files in os.walk(config.root_folder, topdown=True):
        ...
        dirs[:] = [
            d
            for d in dirs
            if not _is_excluded(current_path / d, config.root_folder, config)
        ]
        ...
        display_name = (
            current_path.name
            if str(rel_to_root) != "."
            else f"{config.root_folder.name}/"
        )
    ...
    try:
            ...
            for file_path in tqdm(sorted(content_files), desc="Processing"):
                ...
                try:
                    ...
                    cleaned = clean_content(
                        raw, config, is_latex=file_path.suffix.lower() == ".tex"
                    )
                    if cleaned:
                        f.write(cleaned + "\n")
        ...
        print(
            f"✓ Created '{config.output_filename}' ({os.path.getsize(config.output_filename) // 1024} KB)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clean", action="store_true", help="Enable LaTeX comment and command cleaning"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Scan from script root instead of thesis folder",
    )
    ...
    if args.root:
        cfg.root_folder = Path(args.root).resolve()
    ...
    if args.bib:
        cfg.extensions_to_include.add(".bib")
    if args.md:
        cfg.extensions_to_include.add(".md")
    if args.txt:
        cfg.extensions_to_include.add(".txt")
