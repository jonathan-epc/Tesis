...
def plot_results(outputs, targets, config, save_path):
    ...
    for i in range(num_channels):
        ...
        ax.hist2d(
            targets_np[:, i].flatten(), outputs_np[:, i].flatten(), bins=512, cmap=cm, range=[[ranges[i][1], ranges[i][9]], [ranges[i][1], ranges[i][9]]], 
        )
        ax.set_aspect("equal")
...
def main(config, model_class, hparams):
    # Load the model
    model = load_model(
        f"savepoints/{config['model']['name']}_best_model.pth",
        model_class,
        config,
        hparams,
    )
    ...
    plot_results(targets-outputs, targets, config, "plots/model_results.png")


if __name__ == "__main__":
    model_class = globals()[CONFIG["model"]["class"]]
    hparams = get_hparams(CONFIG)
    CONFIG["model"]["name"] = "FNOb"
