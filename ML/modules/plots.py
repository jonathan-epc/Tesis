...
class PlotManager:
    ...
    def _ensure_directory(self, path: Path | None = None) -> None:
        """
        ...
        try:
            ...
            raise OSError(f"Failed to create directory {target_path}: {e}") from e
    ...
    def save_plot(
        ...
        if invalid_formats:
            raise ValueError(
                f"Invalid format(s): {invalid_formats}. Valid formats: {valid_formats}"
            )
            ...
            try:
                ...
                if fmt.lower() == "pdf":
                    ...
                    if additional_metadata:
                        custom_keys = ",".join(
                            f"{k}:{v}" for k, v in additional_metadata.items()
                        )
    ...
    def list_saved_plots(self, plot_type: str | None = None) -> list[Path]:
        """
...
def plot_scatter(
    ...
    for plot_type in ["predictions", "residuals"]:
        ...
        plot_idx = 0
        for outputs, targets, names in zip(
            all_outputs, all_targets, all_names, strict=False
        ):
    ...
    for i in range(field_outputs.size(1)):
        ...
        display_name = (
            ...
            if long_names and isinstance(long_names.get(field_names[i]), dict)
                ...
                long_names.get(field_names[i], field_names[i])
                if long_names
                else field_names[i]
            )
        )
        ...
        if detailed:
            metric_text = f"MAE: {mae:.4f}{unit_str}    Max Error: {max_error:.4f}{unit_str}    RMSE: {rmse:.4f}{unit_str}\nMAPE: {mape:.2f}%    NRMSE: {nrmse:.4f}    SMAPE: {smape:.2f}%"
            metric_ax.text(
                0,
                0.9,
                metric_text,
        ...
        fig.suptitle(
            f"{title_prefix}{lang['case']} {case_idx} - {display_name} ({field_names[i]}){unit_str}",
            fontsize=14,
        )
...
def calculate_field_analysis_metrics(
    field_outputs, field_targets, field_names, units=None, language="en"
):
    """
    ...
    for i in range(field_outputs.size(1)):
        ...
        if np.any(targets_np != 0):
            ...
            mape_values = np.abs(
                errors_np[safe_targets_mask] / targets_np[safe_targets_mask]
            )
            mape = float(np.mean(mape_values) * 100) if mape_values.size > 0 else np.nan
            smape_values = (
                2
                * np.abs(errors_np[safe_targets_mask])
                / (
                    np.abs(targets_np[safe_targets_mask])
                    + np.abs(outputs_np[safe_targets_mask])
                )
            )
            smape = (
                float(np.mean(smape_values) * 100) if smape_values.size > 0 else np.nan
            )
        else:
        ...
        for case_idx in range(targets_np.shape[0]):
            target_flat, output_flat = (
                targets_np[case_idx].flatten(),
                outputs_np[case_idx].flatten(),
            )
        ...
        spatial_correlation = (
            float(np.nanmean(correlations)) if correlations else np.nan
        )
        # --- MODIFICATION END ---
        ...
        field_metrics = {
            ...
            lang["stat_nrmse"]: (
                nrmse if not np.isnan(nrmse) else "N/A"
            ),  # Keep nrmse as float or "N/A"
            lang["stat_smape"]: f"{smape:.2f}%" if not np.isnan(smape) else "N/A",
            lang["stat_spatial_corr"]: (
                spatial_correlation if not np.isnan(spatial_correlation) else "N/A"
            ),
        }
...
def plot_field_analysis(
    ...
    translations = {
        ...
        "es": {
            ...
            "y_profile_title": "Perfil del Eje Y",
            "x_profile_title": "Perfil del Eje X",
    ...
    for i in range(field_outputs.size(1)):
        ...
        ax_spatial.set_title(
            f"{title_prefix}{lang['spatial_error_dist']}{title_suffix}"
        )
        ax_spatial.set_xlabel("X position")
        ax_spatial.set_ylabel("Y position")
        ...
        cbar_spatial = fig_spatial.colorbar(
            im_spatial, cax=cax, orientation="horizontal"
        )
        ...
        fig_perc, (ax_perc_y, ax_perc_x) = plt.subplots(
            1, 2, figsize=(16, 6)
        )  # Create two subplots
        ...
        profile_data_y = error_q_spatial.mean(
            axis=2
        )  # Shape: (num_percentiles, height)
        Y_y, Q_y = np.meshgrid(np.arange(profile_data_y.shape[1]), q_levels)
        contour_y = ax_perc_y.contourf(
            Y_y, Q_y, profile_data_y, cmap="turbo", levels=20
        )
        ...
        contour_x = ax_perc_x.contourf(
            X_x, Q_x, profile_data_x, cmap="turbo", levels=32
        )
        ...
        fig_perc.suptitle(
            f"{title_prefix}{lang['percentile_plot_title']}{title_suffix}", fontsize=14
        )
